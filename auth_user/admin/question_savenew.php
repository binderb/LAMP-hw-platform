<?php
    ...
    $fetch_data = "SELECT * FROM question_table WHERE question_title = ?";
    if ($stmt = $conn->prepare($fetch_data)) {
    	$stmt->bind_param("s",$question_title);
        $stmt->execute();
    ...
    if (count($duplicate_data) > 0) {
		...
		$create_question = "INSERT INTO question_table (question_tags, question_title, question_data) VALUES (?, ?, ?)";
		if ($stmt = $conn->prepare($create_question)) {
			$stmt->bind_param("sss",$question_tags,$question_title,$question_data);
			$stmt->execute();
			$stmt->store_result();
